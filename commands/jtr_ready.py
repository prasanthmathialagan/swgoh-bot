...
class JTRReadyCommand(Command):

	@asyncio.coroutine
	def execute(self, client, input, inventory, channel):
		if len(input) == 0:
			yield from client.send_message(channel, "Must supply a name. Usage: @greeter jtr-ready celessa")
			return
		...
		table_data = inventory.get_specific_toons_data_for_member(closest_match, ['Rey (Jedi Training)'])
		if len(table_data) == 1:
			yield from client.send_message(channel, closest_match + ' already has a 7* JTR')
			return

		req_toons = ['Finn', 'Rey (Scavenger)', 'BB-8', 'Veteran Smuggler Chewbacca', 'Veteran Smuggler Han Solo']
		...
		if satisfiedCount == 5:
			yield from client.send_message(channel, closest_match + ' does not have JTR yet but has the toons required for unlocking her at 7*')
			return

		yield from client.send_message(channel, closest_match + ' does not have JTR and need to farm following toons to 7* for unlocking her at 7*')
